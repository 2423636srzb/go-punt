...
<div class="card basic-data-table">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="ms-auto d-flex gap-2">
            ...
            <button type="button"
                class="btn btn-sm rounded-pill btn-primary-600 radius-8 px-20 py-11 d-flex align-items-center gap-2"
                data-bs-toggle="modal" data-bs-target="#accountUploadModal">
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <img src=" {{ url($account->account->game->logo) }}" alt="{{ $account->account->game->name }}"
                                class="flex-shrink-0 me-12 radius-8" width="50" />
                            <h6 class="text-md mb-0 fw-medium flex-grow-1">{{ $account->account->game->name }}</h6>
                        </div>
                    </td>
                    <td>{{ $account->created_at }}</td>
                    <td>{{ $account->assigned_count }}</td>
                    <td>{{ $account->available_count }}</td>
                    <td>
                        <span
                            class=" @if($account->account->status == 'active') bg-success-focus text-success-main @else bg-danger-focus text-danger-main @endif px-24 py-4 rounded-pill fw-medium text-sm"
                            id="game-status-{{ $account->id }}">
                    ...
                    <td>
                        ...
                        <div class="form-switch switch-success d-inline-flex align-items-center">
                            <input class="form-check-input switch-input" type="checkbox" data-id="{{ $account->id }}"
                                role="switch" @if($account->status == 'active') checked @endif>
                        </div>
                        <a href="{{ route('games.edit', $account) }}"
...
<div class="modal fade" id="accountUploadModal" tabindex="-1" aria-labelledby="addTaskModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title text-xl mb-0" id="addTaskModalLabel"><span class="game-name"></span></h6>
            ...
            <div class="modal-body">
                <form class="row gy-3 needs-validation" action="{{ route('games.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="account_name" id="" class="form-control" required>
                            <option disabled>Select the game </option>
                        ...
                        <div class="invalid-feedback">
                            Please select the Accunt Name.
                        </div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Username</label>
                        <input type="text" name="login_link" id="login_link" class="form-control" placeholder="Enter Username" required>
                        <div class="invalid-feedback">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="login_link" id="login_link" class="form-control" placeholder="Enter Password" required>
                        <div class="invalid-feedback">
            ...
            <div class="modal-footer justify-content-center gap-3">
                <button class="btn btn-primary-600" type="submit">Create Account</button>
                <button type="button"
                    class="border border-danger-600 bg-hover-danger-200 text-danger-600 text-md px-50 py-11 radius-8"
                    data-bs-dismiss="modal">
                    Cancel
